...
class App(ctk.CTk):
    ...
    def criar_tela_usuario_logado(self, email_logado):
        ...
        self.tela_usuario_logado = UsuarioLogado(self, email_logado = email_logado, voltar_incial=self.criar_tela_inicial)
...
class UsuarioLogado(ctk.CTkFrame):
    def __init__ (self, master, email_logado):
        ...
        #Chamamento da classe game e usuariologado
        self.game_manager = Game(master=self.frame_lateral_menu, content_frame=self.frameprincipalmenu, email=self.email_logado)
        self.game_manager.pack(fill='x', pady=5)

        self.user_manager = GerenciarUser (master = self.frame_lateral_menu, content_frame=self.frameprincipalmenu, email=self.email_logado)
...
class Game(ctk.CTkFrame):
    ...
    def resgatar_premio(self):
        ...
        label_pontos_saldo = ctk.CTkLabel(self.content_frame, text=f"Seus pontos atuais: {pontos_atuais} 🌟",
        ...
        def realizar_resgate(premio_selecionado):
            ...
            if pontos_disp >= custo_premio:
                ...
                label_pontos_saldo.configure(text=f"Seus pontos atuais: {dados_pontos[self.email]} 🌟")

        for premio in premios_disponiveis:
            # (criação dos widgets do prêmio)
            botao_resgatar = ctk.CTkButton(premio_frame, text="Resgatar", 
                                          command=lambda p=premio: realizar_resgate(p))
            botao_resgatar.pack(...)
...
class GerenciarUsuario(ctk.CTkFrame):
        ...
        def __init__ (self, master, content_frame, email, **kwargs):
            super().__init__(master, **kwargs)
            self.content_frame=content_frame
            self.email=email
            self.configure(fg_color="white")
        ...
        def mostrar_dados(self):
            self.limpar_conteudo()
            label = ctk.CTkLabel(self.content_frame, text="Funcionalidade: Mostrar Dados\n(A ser implementada)", font=("Arial", 18))
            label.pack(pady=20)

        def atualizar_dados(self):
            self.limpar_conteudo()
            label = ctk.CTkLabel(self.content_frame, text="Funcionalidade: Atualizar Dados\n(A ser implementada)", font=("Arial", 18))
            label.pack(pady=20)

        def deletar_conta(self):
            self.limpar_conteudo()
            label = ctk.CTkLabel(self.content_frame, text="Funcionalidade: Deletar Conta\n(A ser implementada)", font=("Arial", 18))
            label.pack(pady=20)

        def feedback(self):
            self.limpar_conteudo()
            label = ctk.CTkLabel(self.content_frame, text="Funcionalidade: Enviar Feedback\n(A ser implementada)", font=("Arial", 18))
            label.pack(pady=20)
